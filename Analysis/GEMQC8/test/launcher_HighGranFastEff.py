...
if __name__ == '__main__':
    ...
    # Define the parser
    import argparse
    parser = argparse.ArgumentParser(description="QC8 data analysis step 4. High granularity fast efficiency calculation. For any doubt: https://twiki.cern.ch/twiki/bin/view/CMS/GEMCosmicRayAnalysis")
    # Positional arguments
    parser.add_argument("run_number", type=int, help="Specify the run number")
    args = parser.parse_args()

    # Different paths definition
    srcPath = os.path.abspath("launcher_HighGranFastEff.py").split('QC8Test')[0]+'QC8Test/src/'
    pyhtonModulesPath = os.path.abspath("launcher_HighGranFastEff.py").split('QC8Test')[0]+'QC8Test/src/Analysis/GEMQC8/python/'
    runPath = os.path.abspath("launcher_HighGranFastEff.py").split('QC8Test')[0] + 'QC8Test/src/Analysis/GEMQC8/test/'
    configTablesPath = os.path.abspath("launcher_HighGranFastEff.py").split('QC8Test')[0] + 'QC8Test/src/Analysis/GEMQC8/data/StandConfigurationTables/'
    alignmentTablesPath = os.path.abspath("launcher_HighGranFastEff.py").split('QC8Test')[0] + 'QC8Test/src/Analysis/GEMQC8/data/StandAligmentTables/'
    resDirPath = os.path.abspath("launcher_HighGranFastEff.py").split('QC8Test')[0]
    ...
    runCommand = "cmsRun -n 8 runGEMCosmicStand_HighGranFastEff.py"
    ...
    outDirName = "Results_QC8_HighGranFastEff_run_"+str(args.run_number)
    ...
    effoutDir = os.path.abspath("launcher_HighGranFastEff.py").split('QC8Test')[0] + outDirName
    for i in range (0,30):
    ...
    mvToDirCommand = "mv HighGranFastEff_" + out_name + " " + resDirPath+outDirName + "/HighGranFastEff_" + out_name
    ...
    effCommand = "root -l -q -b " + runPath + "macro_HighGranFastEff.c(" + str(args.run_number) + ",\"" + configTablesPath + "\")"
